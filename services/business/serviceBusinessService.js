...
class ServiceBusinessService extends BaseService {
    constructor() {
        super(ServiceInquiry);
    }

    generateSmartEmailHtml({ heading, subtitle, contentHtml, callToAction }) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    async submitInquiry(body) {
        ...
        const inquiry = await super.create({
            name,
            email,
            phone: phone || '',
            serviceType,
        ...
        try {
            ...
            const adminHtml = this.generateSmartEmailHtml({
        ...
        try {
            const clientHtml = this.generateSmartEmailHtml({
    ...
    async fetchInquiries() {
        const { data: inquiries, source } = await super.getAll({}, 600, 'all-inquiries');
        const sorted = [...inquiries].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        return { source, inquiries: sorted };
    }

    async replyInquiry(id, replyMessage, status) {
        const { data: inquiryObj } = await super.getById(id, 600);
        if (!inquiryObj) {
        ...
        const updatedInquiry = await super.update(id, updateData);

        const clientReplyHtml = this.generateSmartEmailHtml({
      ...
      `,
        ...
        try {
            ...
            throw new AppError(`Inquiry saved, but failed to send email to client: ${emailError.message}`, 500);
        }
    ...
    async fetchInquiryById(id) {
        const { data: inquiry, source } = await super.getById(id, 600);
        if (!inquiry) {
            throw new AppError('Service inquiry not found', 404);
        }
        return { source, ...inquiry.toObject ? inquiry.toObject() : inquiry };
    }

    async fetchUserInquiries(userId) {
        const { data: inquiries, source } = await super.getAll({ user: userId }, 600, `my-inquiries-${userId}`);
        const sorted = [...inquiries].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        return {
            success: true,
            source,
...
module.exports = new ServiceBusinessService();
